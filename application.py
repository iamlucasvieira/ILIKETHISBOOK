...
def book(book):
    res = requests.get("https://www.goodreads.com/book/review_counts.json",
                       params={"key": " 9rvfoa1pLB0faeHA1chiww", "isbns": book})
    ...
    if session.get("user") != None:
        ...
        userhasreview = None
    return render_template("book.html",logged=session.get('logged'),book=book,user=session.get("user"),reviews=review,userhasreview=userhasreview,data=data)
...
def api(book_isbn):
    ...
    reviews = db.execute("SELECT * FROM reviews JOIN signup ON reviews.signup_id = signup.id WHERE book_id=:bookid ORDER BY reviews.id DESC",{"bookid":book.id}).fetchall()
    allreviews ={}
    count =0
    sum = 0
    for review in reviews:
        ...
        allreviews[review.id] = ({"name":review.name,"rating":review.rating,"message":review.message})
    if count >0 :
        avg = sum/count
    else:
        avg = 0
    return jsonify({"booktitle":book.title,"bookauthor":book.author,"booksisbn":book.isbn,"reviewcont":count,"year": book.year,"averagerating":avg,"reviews":allreviews})
if __name__ == "__main__":
    with app.app_context():
        main()

@app.route("/userpage/<string:userid>")
def userpage(userid):
    reviews = db.execute(
        "SELECT * FROM reviews  JOIN books ON reviews.book_id = books.id  WHERE signup_id=:userid ORDER BY reviews.id DESC",
        {"userid": userid}).fetchall()

    return render_template("userpage.html", logged=session.get('logged'), message=None, user=session.get("user"),reviews=reviews)